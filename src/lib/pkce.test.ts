...
describe("pkce", () => {
  ...
  describe("buildAuthorizeUrl", () => {
    it("monta a URL com code_challenge e code_challenge_method=S256, sem o code_verifier", () => {
      const url = buildAuthorizeUrl({
        ...
        redirectUri: "https://blog.unumpeople.com.br/auth/callback",
        codeChallenge: "challenge-abc",
      });

      const parsed = new URL(url);
      expect(parsed.origin + parsed.pathname).toBe("https://auth.unumpeople.com.br/oauth2/authorize");
      expect(parsed.searchParams.get("client_id")).toBe("blog-client-id");
      expect(parsed.searchParams.get("response_type")).toBe("code");
      expect(parsed.searchParams.get("redirect_uri")).toBe("https://blog.unumpeople.com.br/auth/callback");
  ...
  describe("redirectToHostedUI", () => {
    ...
    it("persiste verifier e rota de retorno em sessionStorage e redireciona via window.location.href (não router.push)", async () => {
      ...
      expect(url.searchParams.get("redirect_uri")).toBe("https://blog.unumpeople.com.br/auth/callback");
  ...
  describe("logoutFromHostedUI", () => {
    ...
    it("redireciona pro endpoint /logout do Cognito (nunca /oauth2/authorize) com logout_uri válido", () => {
      ...
      expect(url.searchParams.get("client_id")).toBe("blog-client-id");
      expect(url.searchParams.get("logout_uri")).toBe("https://blog.unumpeople.com.br/");
    });
    ...
    // assíncrono (PKCE), às vezes terminava DEPOIS do logout explícito e
    // sobrescrevia window.location.href, reautenticando o usuário
    // silenciosamente via SSO (o Cognito ainda tinha sessão válida).
